...
daftarSimpata.postSimpata = async (tanggal, cif, nopk, norek, jenis, marketing, pokok, rate, kdhit, tglmulai, lama, adm, provisi, tgljtempo, tgl_alih_bunga, tglvaluta, tgltutup, pot_pokok, pot_bunga, sisapokok, sisabunga, isEdit, kantor, opt) => {
  try {
    let rows;
    if (!isEdit) {
      console.log("ADDING");
      [rows] = await db.query(
        `INSERT INTO simpanan(tanggal, cif, nopk, norek, jenis, marketing, pokok, rate, kdhit, tglmulai, lama, adm, provisi, tgljtempo, tglvaluta, tgltutup, pot_pokok, pot_bunga, sisapokok, sisabunga, kantor, opt) VALUES (STR_TO_DATE('${tanggal}','%d-%m-%Y'), '${cif}', '${nopk}', '${norek}', '${jenis}', '${marketing + " "}', '${pokok}', '${rate}', '${kdhit}', STR_TO_DATE('${tglmulai}','%Y-%m-%d'), '${lama}', '${adm}', '${provisi}', STR_TO_DATE('${tgljtempo}','%d-%m-%Y'), '${tglvaluta}', '${tgltutup}', '${pot_pokok}', '${pot_bunga}', '${sisapokok}', '${sisabunga}', '${kantor}', '${opt}')`
      );
    } else {
      console.log("EDITING");
      [rows] = await db.query(
